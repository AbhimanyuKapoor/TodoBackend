...
public class Task {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.UUID) //If we create a task with null id, JPA creates a new UUID
    @Column(name = "id", updatable = false, nullable = false)
    private UUID id;

    //updatable & nullable default value is true
    ...
    /*
    FetchType.LAZY: TaskList wouldn't be loaded from DB until needed
    taskList variable name matches the argument of mappedBy in 'TaskList' class
     */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "task_list_id") //Foreign Key Column name in 'tasks' table, has id of task_list to which the task belongs
